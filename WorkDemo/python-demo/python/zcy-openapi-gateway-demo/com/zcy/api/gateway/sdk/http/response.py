...
class Response(Request):
    ...
    def get_http_response(self):
        ...
        try:
            ...
            self.__connection = httplib.HTTPConnection(self.get_host(), self.get_port(), False, 30000)
            self.__connection.connect()
            post_data = None
            if self.get_content_type() == constant.CONTENT_TYPE_FORM and self.get_body():
                #post_form_data = self.build_post_form_data(body=self.get_body())
                #post_data = urllib.urlencode(post_form_data)
    ...
    def build_post_form_data(self, body={}):
        resource = []
